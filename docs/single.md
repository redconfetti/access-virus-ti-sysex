...
This outlines the structure of a Single program SysEx dump, based on the
`sysex/singles/base/base.syx` capture of the `-INIT-` patch.
...
## High‑level regions (from `base.syx`)
...
- **~0xF8–0x103 – Patch name and nearby globals**
  - Contains the ASCII patch name `-INIT-` padded with spaces:
    - The ASCII sequence `2d 49 4e 49 54 2d 20 20 20` (`-INIT-   `)
...
- **0x204–0x20D – Trailer metadata + checksum**
  - The last 8 bytes of `base.syx` are:
    - `7f 40 00 01 00 00 44 f7`
  - `F7` is the SysEx terminator; the preceding byte `0x44` appears to be a
    checksum or validation byte, but its exact algorithm is not yet known
    (simple 7‑bit sum of the body does **not** match 0x44).
...
- **Unknown (to be refined with more examples)**
  - Exact mapping from individual offsets to the GUI parameters listed in
    `sysex/singles/base/README.md` (oscillators, filters, matrix, FX, etc.).
  - The precise meaning of the header bytes at offsets 0x08–0x0B (device ID,
    bank, slot, flags, or OS version).
  - The checksum algorithm used to produce the `0x44` value in `base.syx`.

Future analysis will compare additional Single dumps (with small, targeted
parameter changes) against `base.syx` to pin down byte‑level locations for each
parameter group.
