...
Transcribed from [WAF80 — Access Virus SysEx](http://www.waf80.de/virus/sysex.html)
...
```mermaid
flowchart LR
    WAF80["WAF80 classic row\ncmd, page, param"]
    Hypothesis["Treat as hypothesis\nfor TI mk2"]
    Capture["Capture on hardware\npanel or request/dump"]
    Promote["Promote confirmed mapping\nto TI-specific docs"]
    Retain["Keep or mark N/A\nwhen TI differs"]

    WAF80 --> Hypothesis
    Hypothesis --> Capture
    Capture -->|"matches TI behavior"| Promote
    Capture -->|"missing or changed"| Retain
    Promote --> SingleLive["single-live-edit.md"]
    Promote --> MultiLive["multis-live-edit.md"]
    Promote --> GlobalLive["global-live-edit.md"]
    Promote --> DumpDocs["single-dump.md / multis-dump.md"]
```
...
**Current queue:** Filters are promoted to the TI docs; Oscillator 1 is the
active WAF80-driven confirmation area — see
[testing.md — Confirmation queue](testing.md#confirmation-queue-waf80--ti).
...
1. Use [testing.md — Confirmation queue](testing.md#confirmation-queue-waf80--ti):
   work **one LCD menu (category) at a time**, matching
   [single-dump.md](single-dump.md) sections — not random order through this file.
2. Hypothesis: Page **A** index _N_ → live **`cmd=0x70`**, param **`N`** (hex),
   when global Page A = **SysEx**; else **MIDI CC** _N_ ([waf80.md](#midi-control-change-page-a-map)).
